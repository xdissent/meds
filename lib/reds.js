...
Query.prototype.end = function(fn){
  ...
  if (limit > 0) aggregate.push(
    {$limit: limit});
  aggregate.push({$project: {_id: true}});

  db.collection(key).aggregate(aggregate, function(err, res){
    if (err) return fn(err);
    fn(null, res.map(function(r){
      return r._id;
    }));
  });
...
Search.prototype.index = function(str, id, fn){
  ...
  keys.forEach(function(word, i){
    docs.push({word: map[word], doc: id, score: counts[word]});
  });
